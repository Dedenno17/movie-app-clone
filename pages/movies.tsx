...
import MoviesContent from '../src/components/UI/ContentsLayout';
...
const Movies: NextPage<MoviesProps> = ({ data }) => {
  ...
  const [movieDataForAnotherTenPages, setMovieDataForAnotherTenPages] =
    useState<[]>([]);
  ...
  useEffect(() => {
    ...
    const fetchMoviesData = async () => {
      try {
        const res = await fetch(
          `https://api.themoviedb.org/3/movie/popular?api_key=639d75e6b806c03213815ae9aa5a9376&language=en-US&page=${currPage}`
        );
        if (!res.ok) {
          throw new Error('Failed to Fetch');
        }
        const data = await res.json();
        setMovieDataForAnotherTenPages(data.results);
        setIsLoading(false);
      } catch (err) {
        alert(err);
      }
  ...
  return (
    <div className="w-full p-5">
      ...
      <div className="w-full flex items-center justify-center py-2">
        ...
        <span className="text-2xl text-slate-200 w-[25%] text-center md:text-3xl md:w-[20%] lg:text-4xl xl:text-5xl">
          MOVIES
        </span>
        <span className="p-[2px] bg-ternaryGrey w-[35%] rounded-full md:w-[40%]"></span>
      </div>
      ...
      <MoviesContent
        ...
        dataNextLoad={movieDataForAnotherTenPages}
        isLoading={isLoading}
      />
